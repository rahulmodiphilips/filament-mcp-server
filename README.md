...
[![smithery badge](https://smithery.ai/badge/@Boston343/filament-mcp)](https://smithery.ai/server/@Boston343/filament-mcp)
...
To install Filament MCP Server for Claude Desktop automatically via [Smithery](https://smithery.ai/server/@Boston343/filament-mcp):

```bash
npx -y @smithery/cli install @Boston343/filament-mcp --client claude
```
...
```json title="mcp_config.json"
{
	"mcpServers": {
		"filament": {
			"command": "node",
			"args": ["c:\\path\\to\\folder\\filament-mcp\\dist\\server.js"],
			"env": {}
		}
	}
}
...
| `fetch_llm_data`      | Fetches LLM data from filament.eu1.phsdp.com (rate limited, with caching)   |
